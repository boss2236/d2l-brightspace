...
def _rest():
    ...
    def semestra_export(db, p, q):
        from . import semestra
        if "id" in p:
            payload, warnings = semestra.payload(db, p["id"], num(q, "credits", None))
            return {"payload": payload, "warnings": warnings}
        return semestra.all_payloads(db)
    route("/api/semestra")(semestra_export)
    route("/api/semestra/{id:int}")(semestra_export)
...
class TokenGate:
    ...
    def __init__(self, app, token):
        # token: the string, or a function returning the current one (so rotating it takes effect immediately)
    ...
    @staticmethod
    def _same(given, expected: str) -> bool:
        """Constant-time comparison on bytes (str compare_digest raises on non-ASCII input)."""
        if isinstance(given, str):
            given = given.encode("utf-8", "surrogateescape")
        return secrets.compare_digest(given, expected.encode())
    ...
    async def __call__(self, scope, receive, send):
        if scope["type"] == "http":
            ...
            if path.startswith("/c/"):
                secret, sep, rest = path[3:].partition("/")
                if not sep or not self._same(secret, token):
                    return await self._deny(send, 401, b'{"error": "missing or wrong token"}')
                scope = {**scope, "path": "/" + rest, "raw_path": ("/" + rest).encode()}
            elif path != "/health":
                headers = dict(scope.get("headers") or [])
                auth = headers.get(b"authorization", b"")
                qs = dict(p.split("=", 1) for p in scope.get("query_string", b"").decode("latin-1").split("&") if "=" in p)
                if not (self._same(auth, f"Bearer {token}") or self._same(qs.get("token", ""), token)):
                    return await self._deny(send, 401, b'{"error": "missing or wrong token"}')
...
# one comment per setting the app writes itself, added once
NOTES = {
    "D2L_API_TOKEN": "generated by d2l: bearer token for /api and /mcp (rotate in the app or `d2l token --rotate`)",
    "SEMESTRA_URL": "Semestra connector (set in the app: Connect AI -> Semestra)",
    "SEMESTRA_KEY": "Semestra connector key — treat like a password",
}


def _set_env(key: str, value: str) -> None:
    """Set KEY=value in .env (replacing an existing line), written atomically and readable only by you."""
    import re as _re
    ...
    line = f"{key}={value}"
    if any(_re.match(rf"\s*{key}\s*=", ln) for ln in lines):
        lines = [line if _re.match(rf"\s*{key}\s*=", ln) else ln for ln in lines]
    else:
        note = NOTES.get(key)
        lines += ["", f"# {note}", line] if note and f"# {note}" not in lines else ["", line]
...
def token() -> str:
    ...
    _set_env("D2L_API_TOKEN", t)
    import sys
    print("generated D2L_API_TOKEN and saved it to .env", file=sys.stderr)   # stderr: stdout is MCP's channel
    return t


def rotate_token() -> str:
    """Replace the token. Old connector links, n8n credentials and ?token= URLs stop working at once."""
    t = secrets.token_urlsafe(32)
    _set_env("D2L_API_TOKEN", t)
    return t
...
def build_http_app(host: str = "127.0.0.1", public_hosts: list[str] | None = None):
    ...
    token()                                  # make sure one exists before serving
